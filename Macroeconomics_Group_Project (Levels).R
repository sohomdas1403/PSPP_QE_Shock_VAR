...
# Trim interest_r and inflation to match the 2015-04 start of other variables
interest_r  <- interest_r[interest_r$date >= "2015-04", ]
inflation   <- inflation[inflation$date >= "2015-04", ]
...
png("irf_interest_rate_levels.png", width=800, height=600)
plot(irf_interest)
dev.off()
...
irf_yoy <- irf(estimated.var1, 
               ...
               response="yoy",
               n.ahead=24, boot=TRUE, ci=0.95, runs=500)
png("irf_yoy_levels.png", width=800, height=600)
plot(irf_yoy)
dev.off()
...
irf_unemployment <- irf(estimated.var1, 
                        ...
                        n.ahead=24, boot=TRUE, ci=0.95, runs=500)
png("irf_unemployment_levels.png", width=800, height=600)
...
irf_inflation <- irf(estimated.var1, 
                     ...
                     response="inflation",
                     n.ahead=24, boot=TRUE, ci=0.95, runs=500)
png("irf_inflation_levels.png", width=800, height=600)
plot(irf_inflation)
dev.off()
...
irf_psp <- irf(estimated.var1, 
               ...
               response="psp",
               n.ahead=24, boot=TRUE, ci=0.95, runs=500)
png("irf_psp_levels.png", width=800, height=600)
plot(irf_psp)
dev.off()
...
# Plot and Save
png("qe_shocks_levels.png", width=800, height=600)
...
png("acf_qe_shock_levels.png", width=800, height=600)
acf(qe_shock, main="ACF of Clean QE Shock")
dev.off()

png("pacf_qe_shock_levels.png", width=800, height=600)
...
# Helper: add 95% confidence band and line of best fit to plot
add_conf_band <- function(x, y, line_col = "red", band_col = rgb(1, 0, 0, 0.15)) {
  ...
  preds   <- predict(fit, newdata = data.frame(x = x_seq), interval = "confidence")
  polygon(c(x_seq, rev(x_seq)),
          c(preds[, "lwr"], rev(preds[, "upr"])),
          col = band_col, border = NA)
  lines(x_seq, preds[, "fit"], col = line_col, lwd = 2)
}

png("scatter_qe_financial_levels.png", width=1200, height=1000)
par(mfrow=c(2,2))
...
add_conf_band(qe_shock_df$qe_shock, eampd$OIS_2Y)
...
plot(qe_shock_df$qe_shock, eampd$DE10Y,
     ...
     pch=16, col="steelblue")
add_conf_band(qe_shock_df$qe_shock, eampd$DE10Y)
...
plot(qe_shock_df$qe_shock, eampd$IT10Y,
     ...
     pch=16, col="steelblue")
add_conf_band(qe_shock_df$qe_shock, eampd$IT10Y)
...
plot(qe_shock_df$qe_shock, eampd$EURUSD,
     ...
     pch=16, col="steelblue")
add_conf_band(qe_shock_df$qe_shock, eampd$EURUSD)
...
png("scatter_qe_inflation_levels.png", width=800, height=600)
plot(merged_inflation$qe_shock, merged_inflation$hicp_rate,
     xlab="QE Shock (PSP residual)",
     ylab="Inflation Levels",
     main="QE Shock vs Inflation",
     pch=16, col="steelblue")
add_conf_band(merged_inflation$qe_shock, merged_inflation$hicp_rate)
dev.off()

# Helper: extract correlation, t-statistic, and significance stars from cor.test
cor_stats <- function(x, y) {
  ...
  list(r = round(ct$estimate, 3), t = round(ct$statistic, 3), stars = stars)
}

s1 <- cor_stats(eampd$qe_shock,              eampd$OIS_2Y)
s2 <- cor_stats(eampd$qe_shock,              eampd$DE10Y)
s3 <- cor_stats(eampd$qe_shock,              eampd$IT10Y)
s4 <- cor_stats(eampd$qe_shock,              eampd$EURUSD)
s5 <- cor_stats(merged_inflation$qe_shock,   merged_inflation$hicp_rate)

cor_table_levels <- data.frame(
  Variable    = c("OIS 2Y", "German Bund 10Y", "Italian 10Y", "EUR/USD", "HICP Inflation"),
  Correlation = c(s1$r, s2$r, s3$r, s4$r, s5$r),
  T_Statistic = c(s1$t, s2$t, s3$t, s4$t, s5$t),
  Sig         = c(s1$stars, s2$stars, s3$stars, s4$stars, s5$stars)
)
# Note: *** p<0.01, ** p<0.05, * p<0.10

write.csv(cor_table_levels, "correlation_table_levels.csv", row.names=FALSE)
print(cor_table_levels)
